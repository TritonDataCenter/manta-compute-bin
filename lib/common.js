...
exports.mantaParseHeaderOpt = mantaParseHeaderOpt;
...
function mantaParseHeaderOpt(opt) {
	var s = ':';
	var index = opt.indexOf(s);
	if (index === -1) {
		throw new VError('header opt parse failed');
	}
	//Find out if the first occurance of ':' has a space after.
	if (index === opt.indexOf(': ')) {
		s = ': ';
	}
	return ({
		'key': opt.slice(0, index),
		'value': opt.slice(index + s.length)
	});
}
